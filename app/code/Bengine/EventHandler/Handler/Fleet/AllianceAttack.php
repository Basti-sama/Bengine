...
class Bengine_EventHandler_Handler_Fleet_AllianceAttack extends Bengine_EventHandler_Handler_Fleet_Abstract
{
	/**
	 * (non-PHPdoc)
	...
	protected function _execute(Bengine_Model_Event $event, array $data)
	{
		...
		$assault = new Bengine_Assault($event->getDestination(), $event->getDestinationUserId());
		$assault->addParticipant(Bengine_Assault_Participant::ATTACKER_MODE, $event->getUserid(), $event->getPlanetid(), $event->getTime(), $data);
		// Load allied fleets
		$allies = Bengine::getEH()->getFormationFleets($event->getEventid());
		foreach($allies as $ally)
		{
			$assault->addParticipant(Bengine_Assault_Participant::ATTACKER_MODE, $ally->getUserid(), $ally->getPlanetid(), $event->getTime(), $ally->getData());
		}
	...
	protected function _remove(Bengine_Model_Event $event, array $data)
	{
		...
		{
			Core::getQuery()->updateSet("events", array("mode" => 20, "planetid" => $row["destination"], "destination" => $row["planetid"], "time" => TIME + (TIME - $row["start"])), "eventid = '".$row["eventid"]."'");
		}
		$this->sendBack($data);
		return $this;
	}
