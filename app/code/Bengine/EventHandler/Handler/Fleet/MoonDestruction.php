...
class Bengine_EventHandler_Handler_Fleet_MoonDestruction extends Bengine_EventHandler_Handler_Fleet_Abstract
{
	/**
	 * The death star id.
	 *
	...
	protected function _execute(Bengine_Model_Event $event, array $data)
	{
		...
		if($assault->getData("result") == 1 || $assault->getData("result") == 0)
		{
			...
			$msgAttacker = Bengine::getModel("message");
			$msgAttacker->setReceiver($event->getDestinationUserId())
				->setMode(Bengine_Model_Message::FLEET_REPORTS_FOLDER_ID)
