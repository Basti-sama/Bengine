...
class Bengine_EventHandler_Handler_Fleet_Recycling extends Bengine_EventHandler_Handler_Fleet_Abstract
{
	/**
	 * (non-PHPdoc)
	...
	protected function _execute(Bengine_Model_Event $event, array $data)
	{
		...
		if($_row = Core::getDB()->fetch($result))
		{
			...
			{
				Core::getQuery()->update("galaxy", array("metal", "silicon"), array($restMetal, $restSilicon), "galaxy = '".$data["galaxy"]."' AND system = '".$data["system"]."' AND position = '".$data["position"]."'");
			}
		}
