...
class Bengine_Page_Profile extends Bengine_Page_Abstract
{
	/**
	...
	protected function editAction($user_id = 0)
	{
		...
		$profile = Bengine::getCollection("profile");
	...
	{
		$user = Bengine::getModel("user")->load($user_id);
		$profile = Bengine::getCollection("profile");
		$profile->addUserFilter($user_id)
			->addSortIndex()
			->addListFilter();

		$avatar = Bengine::getModel("profile")->loadByCode("AVATAR", $user_id);
		$about = Bengine::getModel("profile")->loadByCode("ABOUT_ME", $user_id);
	 ...
	 *
	 * @param array		$_POST
	 * @param Bengine_Model_Collection_Profile
	 *
	 * @return Bengine_Page_Profile
	 */
	protected function save($post, Bengine_Model_Collection_Profile $profile)
	{
