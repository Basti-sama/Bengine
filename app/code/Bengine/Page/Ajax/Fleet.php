...
class Bengine_Page_Ajax_Fleet extends Bengine_Page_Ajax_Abstract
{
	/**
	 * Main method of sending fleet via Ajax.
	 *
	 * @return Bengine_Page_Ajax_Fleet
	 */
	protected function init()
	{
		Core::getLanguage()->load("Galaxy");

		$events = Bengine::getEH()->getOwnFleetEvents();
		if($events !== false && Bengine::getResearch(14) + 1 <= count($events))
		{
	...
	protected function espionageAction($target)
	{
		...
		if($row = Core::getDB()->fetch($result))
		{
			...
			if($tar = Core::getDB()->fetch($result))
			{
				...
				$data["maxspeed"] = Bengine::getSpeed(38, $row["speed"]);
				$distance = Bengine::getDistance($tar["galaxy"], $tar["system"], $tar["position"]);
				$time = Bengine::getFlyTime($distance, $data["maxspeed"]);

				$data["consumption"] = Bengine::getFlyConsumption($data["ships"][38]["quantity"] * $row["consume"], $distance);

				if(Bengine::getPlanet()->getData("hydrogen") < $data["consumption"])
				{
				...
				Bengine::getEH()->addEvent(11, $time + TIME, Core::getUser()->get("curplanet"), Core::getUser()->get("userid"), $target, $data);
