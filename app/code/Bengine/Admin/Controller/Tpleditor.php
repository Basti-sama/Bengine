<?php
class Bengine_Admin_Controller_Tpleditor extends Bengine_Admin_Controller_Abstract
{
	...
	protected function editAction()
	{
		...
		Core::getTPL()->assign("backlink", Link::get("admin/tpleditor/show/".Core::getRequest()->getGET("1")."/".$this->package, Core::getLanguage()->getItem("Back")));
	...
	protected function showAction()
	{
		...
		if($directory = new DirectoryIterator($this->mode.$this->package))
		{
			foreach($directory as $file)
			{
				...
				{
					$files[]["template"] = Link::get("admin/tpleditor/edit/".Core::getRequest()->getGET("1")."/".$this->package."/".Str::replace(self::TEMPLATE_EXTENSION, "", $file->getFilename()), $file->getFilename());
				}
			}
		}
	...
	protected function indexAction()
	{
		...
		if($directory = new DirectoryIterator(APP_ROOT_DIR."app/templates/"))
		{
			foreach($directory as $file)
			{
				...
				{
					$appTemplates[]["package"] = Link::get("admin/tpleditor/show/2/".$file->getFilename(), $file->getFilename());
				}
			}
		}
