	...
	protected function seekAction()
	{
		...
		if($s)
		{
			...
			while($row = Core::getDB()->fetch($result))
			{
				$id = $row["userid"];
				$sr[$id]["userid"] = $id;
				$sr[$id]["edit"] = Link::get("user/edit/".$id, Core::getLanguage()->getItem("Edit"));
	...
	protected function editAction($userid)
	{
		...
		if($row = Core::getDB()->fetch($result))
		{
			...
			while($_row = Core::getDB()->fetch($_result))
			{
				...
				$membership[$id]["delete"] = Link::get("user/deletefromgroup/".$userid."/".$id, Core::getLanguage()->getItem("Remove"));
			}
